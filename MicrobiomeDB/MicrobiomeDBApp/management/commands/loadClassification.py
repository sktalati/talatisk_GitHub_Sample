...
from MicrobiomeDBApp.models import ClassificationMethod
import sys
    ...
    def handle(self, *args, **options):
        for filename in args:       #for each of the files
            ...
            for classification in classification_list:     #for each row in the file (row is a dict using the header row as the keys and the values in the following rows as the values)
                ...
                classification = ClassificationMethod.createClassification(classification['ID'],classification['NAME'],classification['DESCRIPTION'],classification['CONTACT NAME'], classification['CONTACT EMAIL'])
